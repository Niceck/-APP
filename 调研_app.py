...
def main():
    ...
    if df.empty:
        ...
        df_grouped = df.groupby(['ts_code', 'name'])['rece_org'].apply(lambda x: ', '.join(x.unique())).reset_index()

        # 计算每个股票的 'rece_org' 数量
        df_grouped['rece_org_count'] = df_grouped['rece_org'].apply(lambda x: len(x.split(', ')))

        # 根据 'rece_org_count' 进行降序排序
        df_grouped.sort_values(by='rece_org_count', ascending=False, inplace=True)

        # 重置索引并删除临时的 'rece_org_count' 列
        df_grouped.reset_index(drop=True, inplace=True)
        df_grouped.drop(columns=['rece_org_count'], inplace=True)

        # 输出表格数据到 Streamlit 页面
        st.subheader("机构调研数据表")
        st.dataframe(df_grouped)

        # 保存文件到相对路径的 'date' 文件夹
        if not os.path.exists("date"):
            os.makedirs("date")
        output_file = os.path.join("date", "机构调研.txt")
